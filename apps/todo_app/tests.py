...
class ToDoUpdateTests(APITestCase):
    ...
    def test_put_update_reminders_various_scenarios(self):
        now = timezone.now()
        self.todo.deadline = now + timedelta(minutes=60)
        self.todo.save()

        pending_future = Notification.objects.create(
            user=self.dean,
            todo=self.todo,
            title='Напоминание о задаче',
            message='pending future',
            type=Notification.Type.TELEGRAM,
            status=Notification.Status.PENDING,
            scheduled_for=self.todo.deadline - timedelta(minutes=30),
            celery_task_id='celery-pending-future'
        )
        ...
        delivered = Notification.objects.create(
            ...
            scheduled_for=self.todo.deadline - timedelta(minutes=50),
            celery_task_id=None
        )

        overdue_pending = Notification.objects.create(
            user=self.dean,
            todo=self.todo,
            title='Напоминание о задаче',
            message='overdue pending',
            type=Notification.Type.TELEGRAM,
            status=Notification.Status.PENDING,
            scheduled_for=now - timedelta(minutes=5),
            celery_task_id='celery-overdue'
        )
        ...
        self.client.force_authenticate(user=self.dean)
        with patch("apps.todo_app.views.sync_calendars") as mock_sync:
            resp = self.client.put(url, {"title": "No reminders change"}, format="json")
        self.assertEqual(resp.status_code, 200)
        pending_future.refresh_from_db()
        delivered.refresh_from_db()
        overdue_pending.refresh_from_db()
        self.assertEqual(pending_future.status, Notification.Status.PENDING)
        self.assertEqual(delivered.status, Notification.Status.SENT)
        self.assertEqual(overdue_pending.status, Notification.Status.PENDING)

        with patch("apps.todo_app.views.sync_calendars") as mock_sync:
            resp = self.client.put(url, {"reminders": None}, format="json")
        self.assertEqual(resp.status_code, 200)
        pending_future.refresh_from_db()
        self.assertEqual(pending_future.status, Notification.Status.PENDING)

        another_pending = Notification.objects.create(
            user=self.dean,
            todo=self.todo,
            title='Напоминание о задаче',
            ...
            type=Notification.Type.TELEGRAM,
            status=Notification.Status.PENDING,
            scheduled_for=self.todo.deadline - timedelta(minutes=20),
            celery_task_id='celery-another'
        )

        with patch('celery.current_app.control.revoke') as mock_revoke, \
             patch("apps.todo_app.views.sync_calendars") as mock_sync:
            resp = self.client.put(url, {"reminders": []}, format="json")

        self.assertEqual(resp.status_code, 200)

        pending_future.refresh_from_db()
        another_pending.refresh_from_db()
        self.assertEqual(pending_future.status, Notification.Status.FAILED)
        self.assertIsNone(pending_future.celery_task_id)
        self.assertEqual(another_pending.status, Notification.Status.FAILED)
        self.assertIsNone(another_pending.celery_task_id)
        mock_revoke.assert_any_call('celery-pending-future', terminate=False)
        mock_revoke.assert_any_call('celery-another', terminate=False)
        ...
        fresh_pending = Notification.objects.create(
            user=self.dean,
            todo=self.todo,
            title='Напоминание о задаче',
            ...
            type=Notification.Type.TELEGRAM,
            status=Notification.Status.PENDING,
            scheduled_for=self.todo.deadline - timedelta(minutes=40),
        ...
        existing_ids = set(Notification.objects.filter(todo=self.todo, title__icontains='Напоминание о задаче')
        ...
        self.assertEqual(fresh_pending.status, Notification.Status.FAILED)
        self.assertIsNone(fresh_pending.celery_task_id)
        mock_revoke.assert_called_with('celery-fresh', terminate=False)

        all_notifs_after = list(Notification.objects.filter(todo=self.todo, title__icontains='Напоминание о задаче'))
        new_notifs = [n for n in all_notifs_after if n.id not in existing_ids]
        future_new = [n for n in new_notifs if n.scheduled_for and n.scheduled_for > now and n.status !=
                      Notification.Status.SENT]
        self.assertGreaterEqual(len(future_new), 1)
        past_new = [n for n in new_notifs if n.scheduled_for and n.scheduled_for <= now]
        self.assertEqual(len(past_new), 0)
